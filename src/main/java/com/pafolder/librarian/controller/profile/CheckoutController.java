...
public class CheckoutController {
  ...
  public ResponseEntity<MappingJacksonValue> create(
    ...
    if (checkoutRepository.findAllActiveByUserId(user.getId()).size()
        >= MAX_BOOKS_ALLOWED_AT_ONCE) {
      throw new ResponseStatusException(
          HttpStatus.UNPROCESSABLE_ENTITY, BORROWING_PROHIBITED_LIMIT_REACHED);
    }
    Book book =
        bookRepository
            .findById(id)
    ...
    Checkout created =
        checkoutRepository.save(new Checkout(null, user, book, LocalDateTime.now(), null));
