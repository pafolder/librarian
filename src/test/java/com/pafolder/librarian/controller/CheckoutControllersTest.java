...
class CheckoutControllersTest extends BaseTestController {
    ...
    void getOwnCheckouts() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(CheckoutController.REST_URL)
    ...
    void create() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.post(CheckoutController.REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void createWithLimitReached() throws Exception {
        Assertions.assertTrue(mockMvc.perform(MockMvcRequestBuilders.post(CheckoutController.REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .param("id", BOOK_ID_FOR_TEST_CHECKOUT))
    ...
    void checkin() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.put(CheckoutController.REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .param("id", String.valueOf(CHECKOUT_ID_FOR_CHECKIN)))
                .andDo(print())
                .andExpect(status().isNoContent());
        Assertions.assertNotNull(checkoutRepository
                .findById(CHECKOUT_ID_FOR_CHECKIN).orElseThrow().getCheckinDateTime());
    }

    @Test
    @WithUserDetails(value = USER_MAIL)
    void checkinNotOwn() throws Exception {
        Assertions.assertTrue(mockMvc.perform(MockMvcRequestBuilders.put(CheckoutController.REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .param("id", String.valueOf(CHECKOUT_ID_FOR_CHECKIN)))
                .andExpect(status().isUnprocessableEntity())
                .andDo(print())
    ...
    void getAllFromIdToId() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(AdminCheckoutController.REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .param("toId", String.valueOf(MAX_ID)))
                .andDo(print())
                .andExpect(status().isOk());
